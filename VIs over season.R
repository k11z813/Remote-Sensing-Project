...
p1 <- ggplot(data.frame(date = dates, NDVI = NDVI), aes(x = date, y = NDVI)) +
  geom_point(aes(fill = NDVI), shape = 21, color = "black", size = 3.5) + 
  geom_line() +
  ggtitle("NDVI") +
  labs(x = "Date", y = "NDVI") +
  scale_fill_viridis_c(begin =0.4, end= 0.85, option="turbo", direction = -1)
  
p2 <- ggplot(data.frame(date = dates, OSAVI = OSAVI), aes(x = date, y = OSAVI)) +
      geom_point(aes(fill = OSAVI), shape = 21, color = "black", size = 3.5) + 
      geom_line() +
  ggtitle("OSAVI") +
      labs(x = "Date", y = "OSAVI")+
  scale_fill_viridis_c(begin =0.4, end= 0.85, option="turbo", direction = -1)
...
p3 <- ggplot(data.frame(date = dates, NDRE = NDRE), aes(x = date, y = NDRE)) +
        geom_point(aes(fill = NDRE), shape = 21, color = "black", size = 3.5) + 
        geom_line() +
  ggtitle("NDRE") +
        labs(x = "Date", y = "NDRE") +
  scale_fill_viridis_c(begin =0.4, end= 0.85, option="turbo", direction = -1)
...
p4 <- ggplot(data.frame(date = dates, RECI = RECI), aes(x = date, y = RECI)) +
        geom_point(aes(fill = RECI), shape = 21, color = "black", size = 3.5) + 
        geom_line() +
  ggtitle("RECI") +
        labs(x = "Date", y = "RECI") +
  scale_fill_viridis_c(begin =0.4, end= 0.85, option="turbo", direction = -1)
